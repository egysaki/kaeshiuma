  ...
  horses.each do |horse|
      ...
      trs.each do |tr|
        begin
          ...
          horse_race_result = HorseRaceResult.find_or_create_by(horse_id: horse.id, race_info_id: race_info.id, accomplishment_time: accomplishment_time, time_for_3f: time_for_3f, order_of_placing: order_of_placing, passing_info: passing_info, weight: weight, basis_weight: basis_weight, popularity: popularity, post_position: post_position, horse_number: horse_number, margin: margin.to_i, odds: odds, weight_difference: weight_difference, pace: pace)
        rescue => e
          Rails.logger.info e
          p e
          next
